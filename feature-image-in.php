<?php
/**
 * Plugin Name: Feature Image in RSS
 * Plugin URI:  https://github.com/dchenk/feature-image-in-rss
...
if ( !function_exists( 'firss_init' ) ) {
	function firss_init()
	{
		...
		// Main hooks.
		add_filter('the_excerpt_rss', 'firss_featured_images_in_rss', 1000, 1);

		add_filter('the_content_feed', 'firss_featured_images_in_rss', 1000, 1);
	...
	function firss_load_plugin_textdomain()
	{
		...
		load_textdomain('featured-images-for-rss-feeds', WP_LANG_DIR . '/plugins/feature-image-in-rss-' . $locale . '.mo' );
		load_plugin_textdomain('featured-images-for-rss-feeds', false, dirname(plugin_basename(__FILE__)));
	}
	...
	<div class="wrap">
			...
			<table class="form-table firss-settings">
				...
				<tr valign="top">
					...
					<td>
						...
						<p>
							<small><?php
							echo sprintf( __( '(Customize image pixel sizes in <a href="%1$s">Media Options</a>, then <a href="%2$s" target=_blank>Regenerate Thumbnails</a>)', 'featured-images-for-rss-feeds' ), admin_url('options-media.php'), 'https://wordpress.org/plugins/regenerate-thumbnails/' ) ;
							?>
							</small>
						</p>
					</td>
		...
		?></th>
				...
				<tr>
					...
					?></th>
					<td>
						<input name="featured_images_in_rss_padding" type="number" value="<?php
		...
		<ul class="footer-notes">
			<li><?php
			echo sprintf(__( 'Still not seeing images in your feed? Verify featured images are set. Clear your browser and any server caches. Using Feedburner? Be sure to also <a href="%s" target=_blank>Ping Feedburner</a> so it refreshes your feed.', 'featured-images-for-rss-feeds'),
				esc_url('https://feedburner.google.com/fb/a/pingSubmit?bloglink=' . site_url()));
				?></li>
			<li><?php
			echo sprintf(__( 'To view your site’s raw RSS feed click here: <a href="%s/feed/" target=_blank>/feed/</a>', 'featured-images-for-rss-feeds'),
				esc_url(site_url()));
				?></li>
		</ul>
	</div>
<?php
	...
	{
		return array_merge([
			'settings' => '<a href="' . admin_url('admin.php?page=featured-images-for-rss-feeds') . '">Settings</a>',
		], $links);
	}
	...
	function firss_eval_css( $featured_images_in_rss_css )
	{
		...
		// Allow additional styling by filter.
